...
from .goal_types import GoalType,GOAL_TYPE_CLASSES
...
class Goal(models.Model):
    ...
    def clean(self):
        ...
        if goal_type_instance:
            missing_fields = [
                f for f in goal_type_instance.required_goal_data()
                if f not in (self.goal_data or {})
            ]
            if missing_fields:
                raise ValidationError(
                    f"Missing required goal data:{missing_fields}"
                )
...
class Tracker(models.Model):
    goal = models.ForeignKey(to=Goal, blank=True, null=True, on_delete=models.SET_NULL, related_name='trackers')
    ...
    def clean(self):
        ...
        if goal_type_instance:
            missing_fields = [
                f for f in goal_type_instance.required_progress_data()
...
class Review(models.Model):
    domain = models.ForeignKey(to=Domain, on_delete=models.CASCADE, related_name='reviews',blank=True)
    ...
    def average_progress(self):
        trackers = Tracker.objects.filter(domain=self.domain, 
        ...
        if self.quarter: 
            ...
            trackers = trackers.filter(date__year=self.year, date__month__range=(start_month, end_month))
...
class Schedule(models.Model):
    ...
    day = models.PositiveSmallIntegerField(null=True,blank=True,help_text="15 for 15th of the month")
    week_order = models.PositiveSmallIntegerField(null=True,blank=True,help_text="2 for 2nd Wednesday of the month")
    ...
    def clean(self):
        ...
        if self.recurrence == self.Recurrence.DAILY:
            if any([self.week_order, self.weekday, self.day]):
                raise ValidationError("Daily recurrence should not specify weekday , day and week_order.")
        
        elif self.recurrence == self.Recurrence.WEEKLY or self.recurrence == self.Recurrence.BIWEEKLY:
            if self.week_order or self.day:
                raise ValidationError("Weekly and biweekly recurrence should not specify day and week_order.")
            if not self.weekday:
            ...
            if not (self.day or (self.week_order and self.weekday)):
                raise ValidationError("Monthly recurrence must specify either a day or (week_order + weekday).")
        
        elif self.recurrence == self.Recurrence.ONCE:
            if not (self.day or self.weekday or (self.week_order and self.weekday)):
                raise ValidationError("Either a day or a weekday or a (week_order + weekday) must be specified.")
        else:
